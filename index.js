...
async function readPrivateKeys() {
  try {
    ...
    logger.error(`Failed to read wallet.txt: ${error.message}`, { emoji: '❌ ' });
    return [];
  }
}
...
async function processAccount(pk, index, total, proxy = null) {
  ...
  logger.info(chalk.bold.magentaBright(`Start processing`), { emoji: '🚀 ', context });
  ...
  logger.info('check-in di process...', { context });
  console.log('\n');
  await performCheckin(token, proxy, context);

  console.log('\n');
  logger.info('mengerjakan mission sedang process...', { context });
  console.log('\n');
  ...
  console.log('\n');
  logger.info('process voting...', { context });
  console.log('\n');
  ...
  if (quota.remaining > 0) {
    ...
    logger.info('No remaining vote, skip voting.', { context });
  }
...
async function initializeConfig() {
  const useProxyAns = await askQuestion(
    chalk.greenBright('🔌 pake proxy ga bang gw nanya nih? (y/n): ')
  );
  if (useProxyAns.trim().toLowerCase() === 'y') {
    ...
    if (globalProxies.length === 0) {
      ...
      logger.warn('ga ada proxy set dulu kalo mau pake di proxy.txt, lanjut tanpa proxy.', { emoji: '⚠️ ' });
    }
  } else {
...
async function runCycle() {
  ...
  if (pks.length === 0) {
    logger.error('No private keys found in wallet.txt. Exiting cycle.', { emoji: '❌ ' });
    return;
  }
...
async function run() {
  ...
  const pinkPurple = gradient(['pink', 'purple']);

  console.log(pinkPurple(centerText(
    '=== Telegram Channel 🚀 : https://t.me/AIRDROPRSIND123 @AIRDROPRSIND123 ===',
    terminalWidth
  )));
  console.log(pinkPurple(centerText('✪ PLAY AI AUTO DAILY ✪', terminalWidth)));
  console.log('\n');
  ...
  while (true) {
    await runCycle();
    logger.info(
      chalk.bold.yellowBright('Cycle completed. Waiting 24 hours...'),
      { emoji: '🔄 ' }
    );
    await delay(86400);
  }
